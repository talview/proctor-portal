...
// Reached when a Supabase recovery link (from request-password-reset)
// lands here with the session tokens in the URL fragment
// (#access_token=...&type=recovery). Mirrors AcceptInvitePage's exact
// pattern -- same underlying Supabase mechanism (setSession from the hash,
// then updateUser({ password })), just for an existing account choosing a
// new password instead of a brand-new one setting their first.
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Best-effort -- clears must_change_password too, in case this reset link
    // was used instead of the forced first-change flow. Not fatal if it fails.
    try {
      ...
      <div className="bg-surface border border-border rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        {status === 'error' && (
          <div className="text-center">
            ...
            <a href="/forgot-password" className="text-accent text-sm font-semibold">Request a new link</a>
          </div>
        )}

        {status === 'success' && (
          <div className="text-center">
            <p className="text-success text-sm font-semibold mb-4">Password reset successfully.</p>
        ...
        {(status === 'form' || status === 'saving') && (
          <form onSubmit={handleSubmit}>
            <p className="text-sm text-text2 mb-5">Choose a new password for your account.</p>
            ...
            >
              {status === 'saving' ? 'Saving...' : 'Reset Password & Continue'}
            </button>

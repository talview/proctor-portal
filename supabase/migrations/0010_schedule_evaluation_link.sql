-- Add an optional meeting/assessment link to scheduling, writing into the existing
-- (previously unused) proctors.demo_eval_link / assessment_link columns.
--
-- Drop the old 7-parameter signature first -- CREATE OR REPLACE with a different
-- parameter count creates a second overload rather than replacing it, which would
-- leave two ambiguous versions of this function around.
DROP FUNCTION IF EXISTS schedule_evaluation(text, text, text, date, text, numeric, text);
...
BEGIN
  ...
  IF p_eval_type = 'demo' THEN
    UPDATE proctors SET
      ...
      demo_eval_link = COALESCE(p_link, demo_eval_link),
      upd = now()
  ...
  ELSE
    UPDATE proctors SET
      ...
      assessment_link = COALESCE(p_link, assessment_link),
      upd = now()
    WHERE id = p_proctor_id;
  END IF;
...
REVOKE ALL ON FUNCTION schedule_evaluation(text, text, text, date, text, numeric, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION schedule_evaluation(text, text, text, date, text, numeric, text, text) TO authenticated;

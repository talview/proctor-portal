-- Re-applies migration 0006 verbatim. Live testing today hit "Assign ID failed: Proctor
-- already has an ID" for a proctor whose pid was genuinely ''. Root cause: this
-- migration's own history was tracked (via `supabase migration repair`) as already
-- applied to production without its SQL ever actually running there, so the live
-- assign_proctor_id was still the pre-0006 `pid IS NOT NULL` version. CREATE OR REPLACE
-- makes this safe to re-run regardless of what's currently live.
--
-- Original 0006 description, unchanged below: proctors.pid is often an empty string ''
-- rather than NULL for records that haven't been assigned an ID yet (matching the
-- original app's JS truthiness checks like `if (proctor.pid)`), but assign_proctor_id's
-- guard used `IS NOT NULL`, which treats '' as "already has an ID" and incorrectly
-- blocks every assignment. Fix: treat empty string the same as NULL.
